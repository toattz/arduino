...
byte getReset()
{
  ...
  {
    #if DEBUG
      Serial.println("getReset():There is config.json");   
    #endif
    StaticJsonDocument<512> readDoc;
    DeserializationError error = deserializeJson(readDoc, file);
    if(error)
    {
      #if DEBUG
      Serial.print(F("getReset() deserializeJson() failed: "));
      Serial.println(error.f_str());
      #endif
    ...
    {
      resetStatus=readDoc["reset"];
    }
  }
  else
  {
    #if DEBUG
      Serial.println("getReset():No config.json");
    #endif
    resetStatus=3;
  }
  file.close();
  Serial.printf("get reset:%d\n",resetStatus);
  return resetStatus;  
}
...
void setReset(byte value)
{
  ...
  { 
    #if DEBUG
      Serial.println("setReset() file&&file.size()");
    #endif
    StaticJsonDocument<512> readDoc;
    DeserializationError error = deserializeJson(readDoc,readFile);
    readFile.close();
    if (error)
    {
      #if DEBUG
        Serial.println("error");
        Serial.print(F("setReset() deserializeJson() failed: "));
        Serial.println(error.f_str());
      #endif
    }
    else
    {
      ...
      //writeDoc["ssid"]=readDoc["ssid"];
      //writeDoc["password"]=readDoc["password"];
      Serial.printf("set reset value:%d\n",value);
      writeDoc["reset"]=value;
      if(serializeJson(writeDoc,writeFile)==0)
      {
        #if DEBUG
          Serial.println(F("Failed to write to file when set reset"));
        #endif
      }
      writeFile.close();
    }
  ...
  {
    #if DEBUG
      Serial.println("setReset() no file&&file.size()");
    #endif
  }    
}
...
void getJson()
{
  Serial.printf("init:%d\nonboard:%d\nconnected:%d\nreset:%d\n",getInit(),getOnboard(),getConnected(),getReset());
}
